...
import static dashboard.dashboardKasir.profileKS;
import frame.login;
...
public class login_rs extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jPanel3.add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(880, 260, 50, 40));
        ...
        username_res.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                username_resActionPerformed(evt);
            }
        });
        ...
        password_res.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                password_resMouseClicked(evt);
            }
        });
        password_res.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                password_resActionPerformed(evt);
            }
        });
...
//        }
        try {
            String sql = "SELECT * FROM user WHERE username = '" + username_res.getText()
                    + "' AND password='" + password_res.getText() + "'";
            ...
            if (rs.next()) {
                if (rs.getString("akses").equals("admin")) {
                    ...
                    namaLoginAdmin.setText(rs.getString(2));
                    namaLoginAdminField.setText(rs.getString(2));
                    this.dispose();
                } else if (rs.getString("akses").equals("kasir")) {
                    popup_bg_rs.setVisible(true);
                    alertLoginSuccessOutput.Message mms = new alertLoginSuccessOutput.Message(this, true);
                    mms.showAlert();
                    this.id_kasirr = rs.getString(1);
                    session.setId_kasir(id_kasirr);
                    new dashboard.dashboardKasir().setVisible(true);
                    namaLoginKasir.setText(rs.getString(2));
                    profileKS.setText(rs.getString(2));
                    this.dispose();
    ...
    public void rfid() {
        try {
            String sql1 = "SELECT * FROM user where id_user='" + username_res.getText() + "'";
            java.sql.Connection conn = (com.mysql.jdbc.Connection) connection.connect.configDB();
            java.sql.PreparedStatement pst = conn.prepareStatement(sql1);
            java.sql.ResultSet rs = pst.executeQuery(sql1);
            if (rs.next()) {
                if (rs.getString("id_user").equals(username_res.getText()) == (rs.getString("akses").equals("admin"))) {
                    session.setUsername(rs.getString("username"));
                    session.setId_kasir(rs.getString("id_user"));
                ...
                if (rs.getString("id_user").equals(username_res.getText()) == (rs.getString("akses").equals("kasir"))) {
                    session.setUsername(rs.getString("username"));
                    session.setId_kasir(rs.getString("id_user"));
                    popup_bg.setVisible(true);
                    alertLoginSuccessOutput.Message mms = new alertLoginSuccessOutput.Message(this, true);
                ...
                Notification panel = new Notification(this, Notification.Type.WARNING, Notification.Location.CENTER, "Data tidak valid!");
    ...
    private void username_resActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_username_resActionPerformed

        rfid();
    }//GEN-LAST:event_username_resActionPerformed

    private void password_resActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_password_resActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_password_resActionPerformed

    private void password_resMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_password_resMouseClicked
        // TODO add your handling code here:
    }//GEN-LAST:event_password_resMouseClicked
